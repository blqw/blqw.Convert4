...
namespace blqw.Kanai.Dynamic
{
    ...
    public class DynamicList : DynamicObject, IList, IObjectReference//, IObjectHandle, ICustomTypeProvider
    {
        ...
        /// </summary>
        /// <param name="list"> </param>
        public DynamicList(IList list, ConvertSettings settings) : this(list) => _settings = settings;
        ...
        object IList.this[int index]
        {
            get
            {
                ...
                {
                    return DynamicFactory.Create(_list[index], _settings);
                }
        ...
        IEnumerator IEnumerable.GetEnumerator()
        {
            foreach (var item in _list)
            {
                ...
                yield return DynamicFactory.Create(item, _settings);
            }
        }
        ...
        {
            if (typeof(IConvertible).IsAssignableFrom(binder.ReturnType) && _list.Count == 1)
            {
                var r = _list[0].Convert(binder.ReturnType, _settings);
                if (r.Success)
                {
            ...
            {
                var r = _list.Convert(binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = r.OutputValue;
                    return true;
        ...
        public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object result)
        {
            ...
            if (i >= 0)
            {
                var r = _list[i].Convert(binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = DynamicFactory.Create(r.OutputValue, _settings);
                    return true;
                }
            }
