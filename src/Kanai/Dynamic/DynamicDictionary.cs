    ...
    public class DynamicDictionary : DynamicObject, IDictionary, IObjectReference//, IObjectHandle, ICustomTypeProvider
    {
        ...
        public override bool TryConvert(ConvertBinder binder, out object result)
        {
            if (typeof(IConvertible).IsAssignableFrom(binder.ReturnType) && (_dict.Count == 1))
            {
                ...
                ee.MoveNext();
                {
                    var r = Cube.Convert(ee.Current, binder.ReturnType, _settings);
                    if (r.Success)
                    {
            ...
            {
                var r = Cube.Convert(_dict, binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = r.OutputValue;
                    return true;
        ...
        public DynamicDictionary(IDictionary dict, ConvertSettings settings) : this(dict) => _settings = settings;
