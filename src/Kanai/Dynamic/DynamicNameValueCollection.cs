...
namespace blqw.Kanai.Dynamic
{
    ...
    public class DynamicNameValueCollection : DynamicObject, IObjectReference//, IObjectHandle, ICustomTypeProvider
    {
        ...
        public DynamicNameValueCollection(NameValueCollection nv, ConvertSettings settings) : this(nv) => _settings = settings;
        ...
        {
            if (typeof(IConvertible).IsAssignableFrom(binder.ReturnType) && _items.Count == 1)
            {
                var r = Cube.Convert(_items[0], binder.ReturnType, _settings);
                if (r.Success)
                {
            ...
            {
                var r = Cube.Convert(_items, binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = r.OutputValue;
                    return true;
