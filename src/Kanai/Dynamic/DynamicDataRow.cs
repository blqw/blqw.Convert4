...
namespace blqw.Kanai.Dynamic
{
    ...
    public class DynamicDataRow : DynamicObject, IObjectReference//, IObjectHandle, ICustomTypeProvider
    {
        ...
        public DynamicDataRow(DataRow row, ConvertSettings settings) : this(row) => _settings = settings;
        public DynamicDataRow(DataRowView row, ConvertSettings settings) : this(row) => _settings = settings;
        ...
        public override bool TryConvert(ConvertBinder binder, out object result)
        {
            if (typeof(IConvertible).IsAssignableFrom(binder.ReturnType))
            {
                ...
                if (arr.Length == 1)
                {
                    var r = Cube.Convert(arr[0], binder.ReturnType, _settings);
                    if (r.Success)
                    {
            ...
            {
                var r = Cube.Convert(_row, binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = r.OutputValue;
                    return true;
            ...
            {
                var r = Cube.Convert(_row.ItemArray, binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = r.OutputValue;
                    return true;
        ...
        public override bool TryGetMember(GetMemberBinder binder, out object result)
        {
            ...
            if (result != null)
            {
                ...
                var r = Cube.Convert(result, binder.ReturnType, _settings);
                if (r.Success)
                {
                    result = r.OutputValue;
                    return true;
        ...
        public override bool TrySetIndex(SetIndexBinder binder, object[] indexes, object value)
        {
            ...
            if (index is string name)
            {
                if (_row.Table.Columns.Contains(name))
                {
                    ...
                    var r = Cube.Convert(value, col.DataType, _settings);
                    if (r.Success)
                    {
                        _row[col] = r.OutputValue;
                        return true;
                    }
                }
            }
